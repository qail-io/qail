...
- **Empty UUID GUCs always emit the nil UUID.** `context_to_sql*` substituted `00000000-…` for an absent `tenant_id`/`agent_id` only on global contexts; a user-only or tenant-only context set `''`, which any `current_setting(...)::uuid` policy turns into a query-time cast error (PostgreSQL does not short-circuit `OR`). All three UUID-shaped GUCs are now nil when absent.
