...
- Nested-relation traversal covers `OnConflict.where_conditions`, so a subquery inside `ON CONFLICT … WHERE` on a registered table is scoped.
  ...
  **Behavior change:** `with_rls()` on a tenant-registered table with `RlsContext::empty()`, `RlsContext::agent(..)` or `RlsContext::user(..)` is now an error instead of an unscoped query.
