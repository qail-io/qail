...
let q = Qail::merge_into("orders")
    ...
    .merge_on_column("o.id", Operator::Eq, "s.order_idd");
"#;
    let mut usages = Vec::new();
    scan_file("test.rs", content, &mut usages);
    let diagnostics = validate_against_schema_diagnostics(&schema, &usages);
    ...
    assert!(
        diagnostics.iter().any(|d| d.message.contains("order_idd")),
        "inline using_table aliases should validate source columns against the aliased table: {:?}",
        diagnostics
    );
}
...
#[test]
fn test_super_admin_audit_accepts_filter_cond_tenant_id_eq_scope() {
    let schema = Schema::parse(
        r#"
table orders {
  id UUID
...
fn demo(tenant_id: uuid::Uuid) {
    ...
    let _q = Qail::get("orders")
        .columns(["id"])
        .filter_cond(eq("tenant_id", tenant_id));
}
"#;
    let mut usages = Vec::new();
    scan_file("test.rs", source, &mut usages);
    ...
    assert!(
        !diagnostics
            .iter()
            .any(|d| d.message.contains("no explicit tenant scope")),
        "tenant_id filter_cond equality should count as explicit tenant scope: {:?}",
        diagnostics
    );
}

#[test]
fn test_super_admin_audit_accepts_filter_cond_condition_struct_tenant_scope() {
    let schema = Schema::parse(
        r#"
table orders {
  id UUID
...
"#;
    ...
    assert!(
        !diagnostics
            .iter()
            .any(|d| d.message.contains("no explicit tenant scope")),
        "tenant_id Condition struct equality should count as explicit tenant scope: {:?}",
        diagnostics
    );
}
...
#[test]
fn test_super_admin_audit_rejects_function_condition_struct_tenant_scope() {
    let schema = Schema::parse(
        r#"
table orders {
  id UUID
...
"#;
    ...
    assert!(
        ...
        "computed tenant_id function expressions must not count as explicit tenant scope: {:?}",
        diagnostics
    );
}
