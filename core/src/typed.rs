...
impl<T: Table> TypedQail<T> {
    ...
    pub fn filter(
        ...
        op: crate::ast::Operator,
        value: impl Into<crate::ast::Value>,
    ) -> Self {
...
mod tests {
    ...
    impl AsRef<str> for Posts {
        fn as_ref(&self) -> &str {
            "posts"
        }
    }
    impl From<Posts> for String {
        fn from(_: Posts) -> String {
            "posts".into()
        }
    }
    ...
    impl AsRef<str> for Products {
        fn as_ref(&self) -> &str {
            "products"
        }
    }
    impl From<Products> for String {
        fn from(_: Products) -> String {
            "products".into()
        }
    }
    ...
    fn test_typed_qail_join_related() {
        ...
        let query = Qail::typed(Users).join_related(Posts).build();

        let sql = query.to_sql();
        assert!(
            sql.contains("LEFT JOIN posts"),
            "Should have LEFT JOIN posts"
        );
    ...
    fn test_typed_qail_reverse_join() {
        ...
        let query = Qail::typed(Posts).join_related(Users).build();

        let sql = query.to_sql();
        assert!(
            sql.contains("LEFT JOIN users"),
            "Should have LEFT JOIN users"
        );
    }
    ...
    fn test_typed_qail_typed_columns_batch() {
        ...
        let query = Qail::typed(Users).typed_columns(vec![col1, col2]).build();
    ...
    fn test_non_rls_table_builds_directly() {
        ...
        let query = Qail::typed(Users).column("email").build();
