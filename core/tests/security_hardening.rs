...
fn injection_single_quote_in_string_value() {
    let cmd = select_where("users", "name", Operator::Eq, Value::String("O'Brien".into()));
    let sql = cmd.to_sql();
    assert!(sql.contains("O''Brien"), "Single quote not escaped: {}", sql);
    assert!(!sql.contains("O'Brien'"), "Unescaped quote breaks syntax: {}", sql);
}

#[test]
fn injection_double_single_quote_in_string() {
    let cmd = select_where("users", "bio", Operator::Eq, Value::String("it''s fine".into()));
    let sql = cmd.to_sql();
    assert!(sql.contains("it''''s fine"), "Double-escaped quotes: {}", sql);
}

#[test]
fn injection_semicolon_in_string_value() {
    let cmd = select_where(
        "users", "name", Operator::Eq,
        Value::String("alice'; DROP TABLE users; --".into()),
    );
    let sql = cmd.to_sql();
    assert!(sql.contains("alice''; DROP TABLE users; --"),
        "Semicolon injection not neutralized: {}", sql);
}

#[test]
fn injection_comment_in_string_value() {
    let cmd = select_where("users", "name", Operator::Eq, Value::String("admin'--".into()));
    let sql = cmd.to_sql();
    assert!(sql.contains("admin''--"), "Comment injection not escaped: {}", sql);
}
...
fn injection_block_comment_in_string_value() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("admin''/**/OR 1=1"), "Block comment injection: {}", sql);
}
...
fn injection_union_select_in_string_value() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("'' UNION SELECT"), "Union injection not escaped: {}", sql);
}
...
fn injection_stacked_query_in_identifier() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("\"users; DROP TABLE orders\""),
        "Semicolon in table not quoted: {}", sql);
}
...
fn injection_double_quote_in_identifier() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("\"users\"\"inject\""),
        "Double quote in identifier not escaped: {}", sql);
}

#[test]
fn injection_backslash_in_string_value() {
    let cmd = select_where("users", "path", Operator::Eq, Value::String("C:\\Users\\admin".into()));
    let sql = cmd.to_sql();
    assert!(sql.contains("C:\\Users\\admin"), "Backslash modified: {}", sql);
}

#[test]
fn injection_newline_in_string_value() {
    let cmd = select_where("users", "bio", Operator::Eq, Value::String("line1\nline2".into()));
    let sql = cmd.to_sql();
...
fn injection_crlf_injection() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("'' OR 1=1"), "CRLF injection not escaped: {}", sql);
}
...
fn injection_empty_string_value() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("''"), "Empty string not properly represented: {}", sql);
}
...
fn injection_string_of_only_quotes() {
    let cmd = select_where("users", "name", Operator::Eq, Value::String("'''".into()));
    let sql = cmd.to_sql();
    assert!(sql.contains("''''''"), "Quote-only string not escaped: {}", sql);
}
...
fn injection_dollar_quoting_attempt() {
    ...
    assert!(sql.contains("'$$; DROP TABLE users; $$'"),
        "Dollar quoting not contained: {}", sql);
}
...
fn like_wildcard_percent_in_value() {
    let cmd = select_where("users", "name", Operator::Fuzzy, Value::String("100% complete".into()));
    let sql = cmd.to_sql();
    assert!(sql.contains("ILIKE") || sql.contains("LIKE"), "Must use LIKE: {}", sql);
    assert!(sql.contains("100% complete"), "Value present: {}", sql);
}

#[test]
fn like_underscore_in_value() {
    let cmd = select_where("users", "name", Operator::Fuzzy, Value::String("user_name".into()));
    let sql = cmd.to_sql();
...
fn like_quote_injection_in_fuzzy() {
    let cmd = select_where(
        "users", "name", Operator::Fuzzy,
        Value::String("admin' OR '1'='1".into()),
    );
    let sql = cmd.to_sql();
    assert!(sql.contains("admin'' OR ''1''=''1"),
        "ILIKE quote injection not escaped: {}", sql);
}
...
fn like_backslash_escape_attempt() {
    ...
    assert!(sql.contains("test\\'' OR 1=1 --"),
        "Backslash escape attack: {}", sql);
}

#[test]
fn like_very_long_pattern() {
    let long_pattern = "a".repeat(10_000);
    let cmd = select_where("users", "name", Operator::Fuzzy, Value::String(long_pattern.clone()));
    let sql = cmd.to_sql();
    assert!(sql.len() > 10_000, "Long pattern truncated: len={}", sql.len());
}
...
fn encoding_null_byte_in_identifier_stripped() {
    ...
    let sql = cmd.to_sql();
    assert!(!sql.as_bytes().contains(&0u8), "Null bytes must be stripped");
    assert!(sql.contains("usersinjected"), "Name after stripping: {}", sql);
}
...
fn encoding_null_byte_in_column_name() {
    ...
    let sql = cmd.to_sql();
    assert!(!sql.as_bytes().contains(&0u8), "Null bytes stripped from columns");
}
...
fn encoding_utf8_bom_in_identifier() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("users"), "Table name contains 'users': {}", sql);
}
...
fn encoding_rtl_override_in_identifier() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("\""), "RTL override must trigger quoting: {}", sql);
}
...
fn encoding_zero_width_space_in_identifier() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("\""), "Zero-width space must trigger quoting: {}", sql);
}
...
fn encoding_null_byte_in_string_value() {
    let cmd = select_where("users", "name", Operator::Eq, Value::String("hello\0world".into()));
    let sql = cmd.to_sql();
...
fn encoding_mixed_unicode_scripts() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("Hello 世界 🌍 مرحبا"), "Unicode preserved: {}", sql);
}
...
fn value_float_neg_infinity() {
    let cmd = select_where("stats", "val", Operator::Eq, Value::Float(f64::NEG_INFINITY));
    let sql = cmd.to_sql();
...
fn operator_between_with_injection() {
    ...
    assert!(sql.contains("0''; DROP TABLE orders; --"),
        "BETWEEN bound injection not escaped: {}", sql);
}
...
fn operator_exists_with_subquery() {
    let subquery = Box::new(Qail {
        ...
        cages: vec![filter_cage(vec![
            cond("status", Operator::Eq, Value::String("active".into())),
        ])],
        ..Default::default()
    });
    let cmd = Qail {
...
fn operator_in_with_injection_values() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("user''; DROP TABLE users; --"),
        "IN value injection not escaped: {}", sql);
}
...
fn operator_is_null_no_value_leak() {
    ...
    assert!(!sql.contains("IS NULL NULL"), "Value leaked into IS NULL: {}", sql);
}
...
fn xss_script_tag_in_value() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("alert(''xss'')"), "XSS quotes not escaped: {}", sql);
}
...
fn xss_html_entities_in_identifier() {
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("users&amp;admin") || sql.contains("\"users&amp;admin\""),
        "HTML entities preserved: {}", sql);
}

#[test]
fn xss_css_injection_in_value() {
    let cmd = select_where(
        "users", "style", Operator::Eq,
        Value::String("background:url(javascript:alert(1))".into()),
    );
    let sql = cmd.to_sql();
    assert!(sql.contains("background:url(javascript:alert(1))"),
        "CSS injection stored: {}", sql);
}
...
fn combined_identifier_and_value_injection() {
    let cmd = select_where(
        "users\"; DROP TABLE x; --", "name", Operator::Eq,
        Value::String("'; DELETE FROM users; --".into()),
    );
    let sql = cmd.to_sql();
    assert!(sql.contains("\"users\"\"; DROP TABLE x; --\""),
        "Table injection not escaped: {}", sql);
    assert!(sql.contains("''; DELETE FROM users; --"),
        "Value injection not escaped: {}", sql);
}
...
fn combined_join_injection() {
    let cmd = Qail {
        ...
        joins: vec![Join {
            ...
            kind: JoinKind::Inner,
            on: Some(vec![
                cond("orders.user_id", Operator::Eq, Value::Column("users; DROP TABLE orders.id".to_string())),
            ]),
            on_true: false,
        }],
        ..Default::default()
    ...
    let sql = cmd.to_sql();
    assert!(sql.contains("\"users; DROP TABLE orders\""),
        "JOIN table injection not quoted: {}", sql);
}
...
fn combined_update_payload_injection() {
    let cmd = Qail {
        ...
        cages: vec![
            payload_cage(vec![
                cond("name", Operator::Eq, Value::String("admin'; UPDATE users SET role='superadmin' WHERE '1'='1".into())),
            ]),
            filter_cage(vec![
    ...
    assert!(sql.contains("admin''; UPDATE users SET role=''superadmin'' WHERE ''1''=''1"),
        "UPDATE payload injection not escaped: {}", sql);
}
...
fn combined_delete_where_injection() {
    let cmd = Qail {
        ...
        table: "orders".to_string(),
        cages: vec![filter_cage(vec![
            cond("id", Operator::Eq, Value::String("1' OR '1'='1".into())),
        ])],
        ..Default::default()
    };
    let sql = cmd.to_sql();
    assert!(sql.starts_with("DELETE"), "Must be DELETE: {}", sql);
    assert!(sql.contains("1'' OR ''1''=''1"),
        "DELETE WHERE injection not escaped: {}", sql);
}
...
fn combined_insert_values_injection() {
    let cmd = Qail {
        ...
        cages: vec![payload_cage(vec![
            cond("name", Operator::Eq, Value::String("alice'), ('evil_user".into())),
        ])],
        ..Default::default()
    };
    let sql = cmd.to_sql();
    assert!(sql.contains("INSERT"), "Must be INSERT: {}", sql);
    assert!(sql.contains("alice''), (''evil_user"),
        "INSERT values injection not escaped: {}", sql);
}
