...
impl Metrics {
    ...
    fn record(&self, d: Duration, status: Option<u16>) {
        ...
        match status {
            Some(s) if (200..400).contains(&s) => { self.success.fetch_add(1, Ordering::Relaxed); }
            Some(s) if (400..500).contains(&s) => { self.errors_4xx.fetch_add(1, Ordering::Relaxed); }
            Some(_) => { self.errors_5xx.fetch_add(1, Ordering::Relaxed); }
            None => { self.errors_net.fetch_add(1, Ordering::Relaxed); }
        }
    }
    ...
    fn report(&self, label: &str, wall_time: Duration) -> Stats {
        ...
        println!("\n  ━━━ {} ━━━", label);
        println!("  Requests: {} | ok={} | 4xx={} | 5xx={} | net-err={}", total, ok, e4, e5, en);
        ...
        println!("  Throughput: {:.0} QPS | Wall: {:.2}s", qps, wall_time.as_secs_f64());

        Stats { label: label.to_string(), qps, p50, p95, p99, ok, errors: e4 + e5 + en }
    }
}
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("║  Duration:    {:<50}║", format!("{}s per phase", config.duration.as_secs()));
    println!("╚══════════════════════════════════════════════════════════════════╝");

    // Health checks
    print!("\n⏳ Engine health... ");
    match client.get(format!("{}/health", config.engine_url)).send().await {
        Ok(r) if r.status().is_success() => println!("✅ OK"),
        Ok(r) => { println!("❌ {}", r.status()); return Err("Engine health failed".into()); }
        Err(e) => { println!("❌ {}", e); return Err("Cannot reach engine".into()); }
    }
    print!("⏳ Gateway health... ");
    match client.get(format!("{}/health", config.gateway_url)).send().await {
        Ok(r) if r.status().is_success() => println!("✅ OK"),
        Ok(r) => { println!("❌ {}", r.status()); return Err("Gateway health failed".into()); }
        Err(e) => { println!("❌ {}", e); return Err("Cannot reach gateway".into()); }
    }
    ...
    // Warmup both
    println!("  Warming up...");
    let _ = attack("warmup-engine", &config.engine_url, endpoint, 5, Duration::from_secs(2), &client).await;
    let _ = attack("warmup-gateway", &config.gateway_url, endpoint, 5, Duration::from_secs(2), &client).await;

    let mut all_results: Vec<(String, Vec<Stats>)> = Vec::new();

    // Run at different concurrency levels
    for concurrency in [config.concurrency, config.concurrency * 2, config.concurrency * 4] {
        println!("\n  ── {} concurrent workers, {}s ──", concurrency, config.duration.as_secs());

        let engine_stats = attack(
            &format!("Engine REST ({concurrency}c)"),
            &config.engine_url, endpoint,
            concurrency, config.duration, &client,
        ).await;
        ...
        println!("\n  → Gateway is {:.1}× the throughput of Engine at {}c", speedup, concurrency);

        all_results.push((format!("{}c", concurrency), vec![engine_stats, gateway_stats]));
    }
    ...
    let engine_s = attack(
        ...
        &config.engine_url, endpoint2,
        config.concurrency, config.duration, &client,
    ).await;

    let gateway_s = attack(
        "Qail Gateway (paginated)",
        &config.gateway_url, endpoint2,
        config.concurrency, config.duration, &client,
    ).await;

    let speedup2 = gateway_s.qps / engine_s.qps.max(1.0);
    println!("\n  → Gateway is {:.1}× the throughput of Engine (paginated)", speedup2);
    ...
    for (label, stats) in &all_results {
        for s in stats {
            println!("║  {:40} {:>7.0} QPS  p50 {:>8.2}ms  p99 {:>8.2}ms  ok={} err={} ║",
                s.label,
                s.qps,
    ...
    // Paginated
    println!("║  {:40} {:>7.0} QPS  p50 {:>8.2}ms  p99 {:>8.2}ms  ok={} err={} ║",
        engine_s.label, engine_s.qps, engine_s.p50.as_secs_f64() * 1000.0, engine_s.p99.as_secs_f64() * 1000.0, engine_s.ok, engine_s.errors);
    println!("║  {:40} {:>7.0} QPS  p50 {:>8.2}ms  p99 {:>8.2}ms  ok={} err={} ║",
        gateway_s.label, gateway_s.qps, gateway_s.p50.as_secs_f64() * 1000.0, gateway_s.p99.as_secs_f64() * 1000.0, gateway_s.ok, gateway_s.errors);
