...
fn command_is_read_only_for_release(cmd: &qail_core::ast::Qail) -> bool {
    ...
    fn value_is_read_only(value: &Value) -> bool {
        match value {
            ...
            Value::Subquery(query) => command_is_read_only_for_release(query),
    ...
    fn expr_is_read_only(expr: &Expr) -> bool {
        match expr {
            ...
            Expr::SpecialFunction { args, .. } => {
                args.iter().all(|(_, expr)| expr_is_read_only(expr))
            }
            ...
            Expr::Subscript { expr, index, .. } => {
                expr_is_read_only(expr) && expr_is_read_only(index)
            }
            Expr::FieldAccess { expr, .. } => expr_is_read_only(expr),
            Expr::Subquery { query, .. } | Expr::Exists { query, .. } => {
                command_is_read_only_for_release(query)
            }
            Expr::Star
            | Expr::Named(_)
            | Expr::Aliased { .. }
    ...
    action_is_read_only
        ...
        && cmd.merge.as_ref().is_none_or(|merge| {
            let source_is_read_only = match &merge.source {
                ...
                MergeSource::Query { query, .. } => command_is_read_only_for_release(query),
            };
            source_is_read_only
