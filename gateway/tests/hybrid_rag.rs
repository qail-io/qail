...
fn fake_embed(text: &str) -> Vec<f32> {
    ...
    if lower.contains("bali")      { v[0] += 1.0; }
    if lower.contains("lombok")    { v[1] += 1.0; }
    if lower.contains("java")      { v[2] += 1.0; }
    if lower.contains("fast")      { v[3] += 1.0; }
    if lower.contains("ferry")     { v[4] += 1.0; }
    if lower.contains("luxury")    { v[5] += 1.0; }
    if lower.contains("budget")    { v[6] += 1.0; }
    if lower.contains("scenic")    { v[7] += 1.0; }
    // Normalize
...
async fn hybrid_rag_search_flow() {
    ...
    let mut pg = PgDriver::connect_env().await
        .expect("Failed to connect to Postgres — set DATABASE_URL");
    let mut qd = QdrantDriver::connect("localhost", 6334).await
    ...
    )).await.expect("Failed to create PG table");
    println!("  ✓ PG table '{}' created", TABLE);

    // Create Qdrant collection
    qd.create_collection(COLLECTION, VECTOR_DIM, Distance::Cosine, false).await
        .expect("Failed to create Qdrant collection");
    tokio::time::sleep(std::time::Duration::from_millis(300)).await;
    println!("  ✓ Qdrant collection '{}' created (dim={})", COLLECTION, VECTOR_DIM);

    // ── 3. Insert documents into Postgres ───────────────────────────
    println!("\n▸ Phase 3: Inserting documents into Postgres...");
    let documents = vec![
        (1, "Bali Fast Ferry", "Express fast ferry service from Java to Bali. Quick crossing.", "Ketapang", "Gilimanuk", 85_000i64),
        (2, "Lombok Scenic Cruise", "Scenic luxury cruise from Bali to Lombok. Beautiful views.", "Padang Bai", "Lembar", 350_000),
        (3, "Java Budget Ferry", "Budget ferry connecting East Java ports. Affordable.", "Surabaya", "Madura", 25_000),
        (4, "Bali Luxury Yacht", "Private luxury yacht charter around Bali. Premium service.", "Benoa", "Nusa Penida", 2_500_000),
        (5, "Fast Lombok Express", "Fast ferry crossing from Bali to Lombok. Speed matters.", "Serangan", "Bangsal", 150_000),
        (6, "Java Scenic Ferry", "Scenic ferry ride along Java northern coast. Enjoy the view.", "Semarang", "Karimunjawa", 180_000),
        (7, "Bali Budget Boat", "Budget boat transfer to Bali islands. Backpacker friendly.", "Sanur", "Nusa Lembongan", 35_000),
    ];
    ...
    // Reset sequence
    pg.execute_raw(&format!("SELECT setval(pg_get_serial_sequence('{}', 'id'), 7)", TABLE)).await.ok();
    ...
    let points: Vec<Point> = documents.iter().map(|(id, title, content, origin, dest, price)| {
    ...
    qd.upsert(COLLECTION, &points, false).await
    ...
    let search_results = qd.search(COLLECTION, &query_embedding, 3, None).await
        .expect("Qdrant search failed");

    println!("  Qdrant returned {} results:", search_results.len());
    let matched_ids: Vec<u64> = search_results.iter().map(|r| {
        let id = match &r.id {
            PointId::Num(n) => *n,
            PointId::Uuid(s) => s.parse().unwrap_or(0),
        };
        println!("    id={}, score={:.4}, title={:?}", id, r.score, r.payload.get("title"));
        id
    }).collect();
    ...
    let id_list = matched_ids.iter().map(|id| id.to_string()).collect::<Vec<_>>().join(",");
    let pg_rows = pg.fetch_raw(&format!(
        "SELECT id, title, content, origin, destination, price_idr FROM {} WHERE id IN ({})",
        TABLE, id_list
    )).await.expect("Postgres query failed");
    ...
    assert_eq!(pg_rows.len(), matched_ids.len(),
    ...
    assert_eq!(matched_ids[0], 1, "Best match should be 'Bali Fast Ferry' (id=1)");
    // id=5 (Fast Lombok Express) should also rank high (has 'fast' + 'bali')
    assert!(matched_ids.contains(&5), "id=5 (Fast Lombok Express) should be in top 3");
    ...
    let ids2: Vec<u64> = results2.iter().map(|r| match &r.id {
        PointId::Num(n) => *n,
        PointId::Uuid(s) => s.parse().unwrap_or(0),
    }).collect();
    println!("  Top results: {:?}", ids2);
    // id=2 (Lombok Scenic Cruise) or id=4 (Bali Luxury Yacht) should be top
    assert!(ids2[0] == 2 || ids2[0] == 4,
        "Top result for 'luxury scenic' should be id=2 or id=4, got {}", ids2[0]);
    ...
    let ids3: Vec<u64> = results3.iter().map(|r| match &r.id {
        PointId::Num(n) => *n,
        PointId::Uuid(s) => s.parse().unwrap_or(0),
    }).collect();
    println!("  Top results: {:?}", ids3);
    assert_eq!(ids3[0], 3, "Top result for 'budget Java' should be id=3 (Java Budget Ferry)");
