...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let main_query = Qail::get("latest_messages")
        ...
        .column_expr(
            coalesce([col("customer_names.customer_sender_name"), text("Unknown")])
                .alias("customer_name"),
        )
        .column_expr(
            coalesce([col("latest_messages.last_message"), text("")]).alias("last_message"),
        )
        ...
        .left_join(
            "customer_names",
            "customer_names.phone_number",
            "latest_messages.phone_number",
        )
        .left_join(
            "unread_counts",
            "unread_counts.phone_number",
            "latest_messages.phone_number",
        )
        .left_join(
            "order_counts",
            "order_counts.phone_number",
            "latest_messages.phone_number",
        )
        .left_join(
            "active_sessions",
