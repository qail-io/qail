...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🔗 Connecting to example_staging...");

    let mut driver = PgDriver::connect("localhost", 5432, "orion", "example_staging").await?;
    ...
    println!(
        "  Cache capacity: {} (reduced from 1000 to prevent OOM)",
        cap
    );
