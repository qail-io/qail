...
fn main() {
    ...
    let dml_tests: Vec<(&str, Qail)> = vec![
        ...
        ("GET with columns", Qail::get("users").columns(vec!["id", "name"])),
        ("GET with filter", Qail::get("users").filter("id", Operator::Eq, Value::String("abc".into()))),
        ...
        ("GET with sort desc", Qail::get("users").order_desc("created_at")),
        ("CNT basic", {
        ...
        ("ADD basic", Qail::add("users").set_value("name", Value::String("test".into()))),
        ("ADD multiple values", {
            Qail::add("users")
        ...
        ("SET basic", Qail::set("users")
            .set_value("name", Value::String("updated".into()))
            .filter("id", Operator::Eq, Value::String("abc".into()))),
        ("DEL basic", Qail::del("users").filter("id", Operator::Eq, Value::String("abc".into()))),
        ...
        // Edge: all operators
        ("GET filter Eq", Qail::get("t").filter("a", Operator::Eq, Value::Int(1))),
        ("GET filter Neq", Qail::get("t").filter("a", Operator::Ne, Value::Int(1))),
        ("GET filter Gt", Qail::get("t").filter("a", Operator::Gt, Value::Int(1))),
        ("GET filter Gte", Qail::get("t").filter("a", Operator::Gte, Value::Int(1))),
        ("GET filter Lt", Qail::get("t").filter("a", Operator::Lt, Value::Int(1))),
        ("GET filter Lte", Qail::get("t").filter("a", Operator::Lte, Value::Int(1))),
        ("GET filter IsNull", Qail::get("t").filter("a", Operator::IsNull, Value::Null)),
        ("GET filter IsNotNull", Qail::get("t").filter("a", Operator::IsNotNull, Value::Null)),
        ("GET filter Like", Qail::get("t").filter("a", Operator::Like, Value::String("%test%".into()))),
        ("GET filter ILike", Qail::get("t").filter("a", Operator::ILike, Value::String("%test%".into()))),
        ("GET filter In", Qail::get("t").filter("a", Operator::In, Value::String("1,2,3".into()))),
        ("GET filter NotIn", Qail::get("t").filter("a", Operator::NotIn, Value::String("1,2,3".into()))),
        // Edge: value types
        ("ADD with null", Qail::add("t").set_value("a", Value::Null)),
        ("ADD with bool true", Qail::add("t").set_value("a", Value::Bool(true))),
        ("ADD with bool false", Qail::add("t").set_value("a", Value::Bool(false))),
        ("ADD with float", Qail::add("t").set_value("a", Value::Float(3.14159))),
        ("ADD with negative int", Qail::add("t").set_value("a", Value::Int(-42))),
        ("ADD with i64 max", Qail::add("t").set_value("a", Value::Int(i64::MAX))),
        ("ADD with i64 min", Qail::add("t").set_value("a", Value::Int(i64::MIN))),
        ("ADD with empty string", Qail::add("t").set_value("a", Value::String("".into()))),
        ("ADD with long string", Qail::add("t").set_value("a", Value::String("x".repeat(10000)))),
        ("ADD with unicode", Qail::add("t").set_value("a", Value::String("中文🚀مرحبا".into()))),
        ("ADD with single quotes", Qail::add("t").set_value("a", Value::String("it's a test".into()))),
        ("ADD with backslash", Qail::add("t").set_value("a", Value::String("path\\to\\file".into()))),
        // Edge: positional params
        ("GET with $1", Qail::get("t").filter("id", Operator::Eq, Value::Param(1))),
        ("GET with $0", Qail::get("t").filter("id", Operator::Eq, Value::Param(0))),
        ("GET with $9999", Qail::get("t").filter("id", Operator::Eq, Value::Param(9999))),
        // Edge: named params
        ("GET with :name", Qail::get("t").filter("id", Operator::Eq, Value::NamedParam("name".into()))),
        // Edge: joins
        ("GET with INNER JOIN", Qail::get("users").join(JoinKind::Inner, "orders", "users.id", "orders.user_id")),
        ("GET with LEFT JOIN", Qail::get("users").join(JoinKind::Left, "orders", "users.id", "orders.user_id")),
        ("GET with RIGHT JOIN", Qail::get("users").join(JoinKind::Right, "orders", "users.id", "orders.user_id")),
        ("GET with FULL JOIN", Qail::get("users").join(JoinKind::Full, "orders", "users.id", "orders.user_id")),
        ("GET with CROSS JOIN", Qail::get("users").join(JoinKind::Cross, "orders", "users.id", "orders.user_id")),
        // Edge: aggregates
        ("GET with COUNT(*)", {
            let mut q = Qail::get("users");
    ...
    println!("  ✅ {}/{} DML encode tests passed (x2 for wire+sql)", pass, dml_tests.len() * 2);
    ...
    let sql_tests: Vec<(&str, Qail, &str)> = vec![
        ...
        ("INSERT", Qail::add("t").set_value("name", Value::String("x".into())), "INSERT"),
        ("UPDATE", Qail::set("t").set_value("name", Value::String("x".into())).filter("id", Operator::Eq, Value::Int(1)), "UPDATE"),
        ("DELETE", Qail::del("t").filter("id", Operator::Eq, Value::Int(1)), "DELETE"),
    ];
    ...
    for (label, cmd, expected_keyword) in &sql_tests {
        ...
        if sql.contains(expected_keyword) {
            ...
            println!("  ❌ {} — SQL missing '{}': {}", label, expected_keyword, &sql[..std::cmp::min(sql.len(), 100)]);
            fail += 1;
        }
    }
    println!("  ✅ {}/{} SQL outputs contain expected keywords", sql_pass, sql_tests.len());
    ...
    println!("  ✅ {}/{} parse→encode integrations passed", integ_pass, integration_queries.len());
