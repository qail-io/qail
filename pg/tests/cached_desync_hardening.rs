...
async fn pipeline_ast_cached_unexpected_backend_invalidates_cache() {
    ...
    let first_cmd = Qail::get("qail_test_subq");
    let err = conn
    ...
    let second_cmd = Qail::get("qail_test_subq");
    let completed = conn
...
async fn pipeline_ast_fast_rejects_data_before_bind() {
    ...
    let err = conn
        .pipeline_execute_count_ast_oneshot(&[Qail::get("qail_test_subq")])
        .await
        .unwrap_err();
    let msg = err.to_string();
    assert!(
...
async fn pipeline_ast_rejects_data_before_bind() {
    ...
    let err = conn
        .pipeline_execute_rows_ast(&[Qail::get("qail_test_subq")])
        .await
        .unwrap_err();
    let msg = err.to_string();
    assert!(
...
async fn pipeline_simple_fast_rejects_data_before_row_description() {
    ...
    let err = conn
        .pipeline_execute_count_simple_ast(&[Qail::get("qail_test_subq")])
        .await
        .unwrap_err();
    let msg = err.to_string();
    assert!(
