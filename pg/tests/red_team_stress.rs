...
fn tierz_recursive_subquery_stack_overflow() {
    ...
    println!("  Recursive AST (5000 deep) produced {} bytes of SQL", sql.len());
...
fn tierz_10k_where_conditions() {
    ...
    let sql = q.to_sql();
    assert!(sql.len() > 100_000, "10k conditions should produce very long SQL");
    ...
    match result {
        ...
        Ok(Err(e)) => {
            ...
            assert!(
                ...
                "Expected timeout error from driver, got: {}", err_msg
            );
...
async fn tierz_slowloris_partial_response() {
    ...
    match result {
        Err(_elapsed) => {
            panic!("Driver hung for 12+ seconds on partial response — Slowloris DoS vulnerability!");
        }
        Ok(Err(e)) => {
            let err_msg = format!("{}", e);
            assert!(
...
fn tierz_bitflip_fuzzing_row_description() {
    ...
    assert_eq!(panics, 0, "Decoder panicked {} times during fuzzing", panics);
    println!("✅ Bit-flip fuzzing: {} positions × 5 flips = {} attempts, 0 panics", 
             valid.len(), valid.len() * 5);
}
...
fn tierz_bitflip_fuzzing_data_row() {
    ...
    println!("✅ DataRow bit-flip fuzzing: {} attempts, 0 panics", valid.len() * 5);
}
...
fn tierz_bitflip_fuzzing_error_response() {
    ...
    println!("✅ ErrorResponse bit-flip fuzzing: {} attempts, 0 panics", valid.len() * 3);
}
...
async fn tierz_connection_exhaustion_100_rapid() {
    ...
    for i in 0..100 {
        let result = qail_pg::PgDriver::connect_url(
            "postgresql://qail_user@localhost:5432/qail_test"
        ).await;
        ...
        match result {
            ...
            Err(e) => {
                ...
                if format!("{}", e).contains("too many") {
                    println!("  ⚠️ Hit max_connections at attempt {} — connection leak!", i);
                    return;
                }
            }
        }
    ...
    let mut driver = qail_pg::PgDriver::connect_url(
        "postgresql://qail_user@localhost:5432/qail_test"
    ).await.expect("Must connect after 100 rapid connections");
    
    let rows = driver.fetch_all(
