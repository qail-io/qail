"""
Pure Python async PostgreSQL driver.

Uses native asyncio for I/O. Rust only provides sync encoding (AstEncoder).
This eliminates the Tokio bridge overhead.
"""
...
def _encode_startup(user: str, database: str) -> bytes:
    """Encode PostgreSQL startup message (pure Python)."""
...
class Row:
    ...
    def to_dict(self) -> dict:
        """Convert row to dict with automatic type conversion."""
        result = {}
        for name, idx in self._name_to_idx.items():
            ...
            if val is None:
                ...
                # Decode UTF-8 and try type conversion
                s = val.decode('utf-8')
                if s == 't':
                    result[name] = True
                elif s == 'f':
    ...
    def __repr__(self):
        return f"Row({len(self._columns)} columns)"
...
class PgDriver:
    ...
    Uses native asyncio for all I/O. Rust provides only sync encoding.
    """
    ...
    async def _handshake(self, user: str, database: str, password: Optional[str]):
        ...
        # Send startup message (pure Python encoding)
            ...
            if msg_type == b'R':  # AuthenticationXXX
                ...
                if auth_type == 0:  # AuthenticationOk
                    ...
                    if password:
                        ...
                        await self._writer.drain()
                elif auth_type == 10:  # SASL (SCRAM-SHA-256)
                    # For now, skip SCRAM - use trust or cleartext
                    raise RuntimeError("SCRAM-SHA-256 not implemented yet. Use trust mode.")
    ...
    async def _read_rows(self) -> list[Row]:
        """Read DataRow messages until ReadyForQuery."""
        rows = []
        col_names = []
    ...
    def _parse_row_description(self, data: bytes) -> list[str]:
        ...
        for _ in range(col_count):
            ...
            names.append(name)
            offset = end + 1 + 18  # Skip null + 18 bytes of metadata
        return names
    ...
    async def pipeline_batch(self, cmds: list[Qail]) -> int:
        ...
        self._writer.write(wire_bytes)
        await self._writer.drain()
        return await self._read_batch_count(len(cmds))
    
    async def _read_batch_count(self, expected: int) -> int:
        """Read batch results, return count."""
        completed = 0
        while True:
            msg_type, data = await self._recv_msg()
    ...
    async def close(self):
        """Close connection."""
        self._writer.write(b'X\x00\x00\x00\x04')  # Terminate
        await self._writer.drain()
        self._writer.close()
