...
async fn test_tls_connection() -> PgResult<()> {
    ...
    let _conn = PgConnection::connect_tls(
...
async fn test_tls_execute() -> PgResult<()> {
    ...
    let create_cmd = qail_core::parse(
        "make qail_tls_test id:serial:pk, message:text, created_at:timestamptz"
    ).unwrap();
...
async fn test_tls_stress() -> PgResult<()> {
    ...
    println!("✅ 100 queries completed in {:?} ({:.2}ms/query)", elapsed, elapsed.as_millis() as f64 / 100.0);
