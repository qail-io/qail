...
async fn main() -> QdrantResult<()> {
    ...
    println!(
        "\n2. Creating collection '{}' (via REST)...",
        COLLECTION_NAME
    );
    ...
    let points = vec![
        qail_qdrant::Point::new_num(1, vec![0.1, 0.2, 0.3, 0.4]).with_payload("name", "Product A"),
        qail_qdrant::Point::new_num(2, vec![0.2, 0.3, 0.4, 0.5]).with_payload("name", "Product B"),
        qail_qdrant::Point::new_num(3, vec![0.9, 0.8, 0.7, 0.6]).with_payload("name", "Product C"),
    ];
    ...
    let results = driver
        .search(COLLECTION_NAME, &query_vector, 3, None)
        .await?;
