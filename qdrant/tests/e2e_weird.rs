...
async fn weird_search_empty_collection() {
    ...
    let results = d.search("weird_empty", &[1.0, 0.0, 0.0], 10, None).await
        .expect("search on empty should not error");
    assert!(results.is_empty(), "Expected 0 results, got {}", results.len());
...
async fn weird_duplicate_id_overwrite() {
    ...
    let p1 = vec![Point::new(1u64, vec![1.0, 0.0, 0.0])
        .with_payload("version", "first")];
    d.upsert("weird_dup", &p1, false).await.unwrap();

    let p2 = vec![Point::new(1u64, vec![0.0, 1.0, 0.0])
        .with_payload("version", "second")];
    d.upsert("weird_dup", &p2, false).await.unwrap();
    tokio::time::sleep(std::time::Duration::from_millis(300)).await;

    // Search should find the overwritten version
    let results = d.search("weird_dup", &[0.0, 1.0, 0.0], 1, None).await.unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].id, PointId::Num(1));
    // The score should be 1.0 (exact match with the second upsert vector)
    assert!((results[0].score - 1.0).abs() < 0.001, "Score should be ~1.0, got {}", results[0].score);
...
async fn weird_unicode_payload() {
    ...
    let points: Vec<Point> = cursed_strings.iter().enumerate().map(|(i, (key, val))| {
        Point::new((i + 1) as u64, vec![1.0, 0.0])
            .with_payload(*key, *val)
    }).collect();

    d.upsert("weird_unicode", &points, false).await
        .expect("Unicode upsert should work");
    tokio::time::sleep(std::time::Duration::from_millis(300)).await;

    // Scroll all and verify payloads survived the round trip
    let scroll = d.scroll("weird_unicode", 20, None, false).await
        .expect("Scroll should work");
    assert_eq!(scroll.points.len(), cursed_strings.len(),
        "Should have {} points, got {}", cursed_strings.len(), scroll.points.len());

    println!("  ✓ All {} Unicode payloads survived round-trip", cursed_strings.len());
...
async fn weird_high_dim_vectors() {
    ...
    let points: Vec<Point> = (0..5).map(|i| {
        let mut vec = vec![0.0f32; 1536];
        // Seed different dimensions for each point
        for j in 0..1536 {
            vec[j] = ((i * 1536 + j) as f32 * 0.001).sin();
        }
        Point::new((i + 1) as u64, vec)
            .with_payload("dim", 1536i64)
    }).collect();

    d.upsert("weird_highdim", &points, false).await
        .expect("1536-dim upsert should work");
    tokio::time::sleep(std::time::Duration::from_millis(500)).await;

    // Search with a 1536-dim query
    let query: Vec<f32> = points[2].vector.clone();
    let results = d.search("weird_highdim", &query, 3, None).await
        .expect("1536-dim search should work");

    assert!(!results.is_empty());
    assert_eq!(results[0].id, PointId::Num(3), "Exact match should be first");
    assert!((results[0].score - 1.0).abs() < 0.01, "Self-search score should be ~1.0");
...
async fn weird_score_threshold() {
    ...
    let all = d.search("weird_threshold", &[1.0, 0.0, 0.0], 10, None).await.unwrap();
    assert_eq!(all.len(), 3);

    // With high threshold: only exact + partial
    let filtered = d.search("weird_threshold", &[1.0, 0.0, 0.0], 10, Some(0.5)).await.unwrap();
    println!("  Filtered results (threshold=0.5): {:?}",
        filtered.iter().map(|p| (&p.id, p.score)).collect::<Vec<_>>());
    assert!(filtered.len() <= 2, "Threshold 0.5 should filter orthogonal");
    assert!(filtered.iter().all(|p| p.score >= 0.5), "All scores should be >= 0.5");

    // With very high threshold: only exact
    let strict = d.search("weird_threshold", &[1.0, 0.0, 0.0], 10, Some(0.99)).await.unwrap();
    assert_eq!(strict.len(), 1, "Only exact match should pass 0.99 threshold");
...
async fn weird_concurrent_pool() {
    ...
    // Seed some data
    let points: Vec<Point> = (0..20).map(|i| {
    ...
    for i in 0..10u32 {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let results = conn.search("weird_concurrent", &query, 3, None).await
    ...
    println!("  ✓ 10 concurrent searches completed ({} total results)", total);
...
async fn weird_uuid_point_ids() {
    ...
    let points = vec![
        Point { id: PointId::Uuid(uuid1.to_string()), vector: vec![1.0, 0.0], payload: HashMap::new() },
        Point { id: PointId::Uuid(uuid2.to_string()), vector: vec![0.0, 1.0], payload: HashMap::new() },
    ];
    d.upsert("weird_uuid", &points, false).await.expect("UUID upsert should work");
    tokio::time::sleep(std::time::Duration::from_millis(300)).await;
...
async fn weird_nested_payload() {
    ...
    nested_obj.insert("street".to_string(), PayloadValue::String("Jl. Sudirman".to_string()));
    nested_obj.insert("city".to_string(), PayloadValue::String("Jakarta".to_string()));
    nested_obj.insert("zip".to_string(), PayloadValue::Integer(12345));

    let mut payload = HashMap::new();
    payload.insert("name".to_string(), PayloadValue::String("PT Qail".to_string()));
    payload.insert("active".to_string(), PayloadValue::Bool(true));
    payload.insert("rating".to_string(), PayloadValue::Float(4.85));
    payload.insert("tags".to_string(), PayloadValue::List(vec![
    ...
    d.upsert("weird_nested", &points, false).await.expect("Nested payload upsert should work");
    tokio::time::sleep(std::time::Duration::from_millis(300)).await;
...
async fn weird_scroll_pagination() {
    ...
    // Insert 15 points
    let points: Vec<Point> = (1..=15).map(|i| {
    ...
    println!("    Page 1: {} points, next_offset={:?}", page1.points.len(), page1.next_offset);

    // Page 2: next 5 using offset
    assert!(page1.next_offset.is_some(), "Should have next_offset for pagination");
    let page2 = d.scroll("weird_paginate", 5, page1.next_offset.as_ref(), false).await.unwrap();
    assert_eq!(page2.points.len(), 5, "Page 2 should have 5 points");
    println!("    Page 2: {} points, next_offset={:?}", page2.points.len(), page2.next_offset);

    // Page 3: last 5
    assert!(page2.next_offset.is_some(), "Should have next_offset for page 3");
    let page3 = d.scroll("weird_paginate", 5, page2.next_offset.as_ref(), false).await.unwrap();
    assert_eq!(page3.points.len(), 5, "Page 3 should have 5 points");
    println!("    Page 3: {} points, next_offset={:?}", page3.points.len(), page3.next_offset);

    // Page 4: should be empty (no more points)
    if let Some(ref offset) = page3.next_offset {
        let page4 = d.scroll("weird_paginate", 5, Some(offset), false).await.unwrap();
    ...
    let mut all_ids: Vec<_> = page1.points.iter()
    ...
    all_ids.sort();
    all_ids.dedup();
    assert_eq!(all_ids.len(), 15, "Should see all 15 unique IDs across pages");
...
async fn weird_reconnection() {
    ...
    for i in 0..5 {
        ...
        match r {
            ...
            Err(e) => println!("    Rapid query {}: error (expected during stress): {}", i, e),
        }
    }

    // After rapid fire, the driver should still work (auto-reconnect)
    let results = d.search("weird_reconnect", &[1.0, 0.0], 1, None).await
...
async fn weird_batch_upsert_1000() {
    ...
    let points: Vec<Point> = (1..=1000).map(|i| {
        let mut vec = vec![0.0f32; 8];
        for j in 0..8 {
            vec[j] = ((i * 8 + j) as f32 * 0.01).sin();
        }
        Point::new(i as u64, vec)
            .with_payload("batch_id", i as i64)
    }).collect();

    d.upsert("weird_batch", &points, false).await
    ...
    let mut offset = None;
    loop {
        let page = d.scroll("weird_batch", 100, offset.as_ref(), false).await.unwrap();
    ...
    println!("  ✓ 1000 points upserted and scrolled in {:?}", start.elapsed());
