...
#[tokio::test]
async fn live_postgres_pool_storage_runtime_guarantees() {
    let Ok(database_url) = std::env::var("QAIL_WORKFLOW_PG_DATABASE_URL") else {
        eprintln!("skipping live test; QAIL_WORKFLOW_PG_DATABASE_URL is not set");
        return;
    };
    ...
    let suffix = Utc::now()
        ...
        .expect("timestamp should fit");
    let tables = PgWorkflowTables {
        states: format!("qail_workflow_pool_states_{suffix}"),
        leases: format!("qail_workflow_pool_leases_{suffix}"),
        operations: format!("qail_workflow_pool_operations_{suffix}"),
        side_effects: format!("qail_workflow_pool_side_effects_{suffix}"),
    };
    ...
    .await
    .unwrap();
    let store = PgWorkflowStore::from_pool(pool).with_tables(tables.clone());
    store.install_schema().await.unwrap();
    let executor = PgWorkflowExecutor::new(NoopExecutor, store);

    let mut ctx = WorkflowContext::new("wf-pool-live", "awaiting_vendor");
    ctx.definition_name = Some("booking_recovery_pool".to_string());
    ctx.set("booking_id", serde_json::json!("booking-pool-1"));
    WorkflowExecutor::save_state(&executor, &ctx).await.unwrap();

    let loaded = WorkflowExecutor::load_state(&executor, "wf-pool-live")
        .await
        .unwrap()
        .expect("saved pooled state should load");
    assert_eq!(loaded.workflow_id, "wf-pool-live");
    assert_eq!(loaded.get_str("booking_id"), Some("booking-pool-1"));

    let lease = WorkflowLease {
        workflow_id: "wf-pool-live".to_string(),
        owner: "worker-pool-a".to_string(),
        ttl: std::time::Duration::from_secs(30),
    };
    assert!(
        WorkflowExecutor::acquire_workflow_lease(&executor, &lease)
    ...
    let operation = WorkflowOperation {
        workflow_name: "booking_recovery_pool".to_string(),
        workflow_id: "wf-pool-live".to_string(),
        idempotency_key: "event-pool-1".to_string(),
        kind: WorkflowOperationKind::Resume {
            event: "vendor.accepted".to_string(),
        },
    };
    ...
    let side_effect = WorkflowSideEffect {
        ...
        step_path: "steps[1]".to_string(),
        kind: WorkflowSideEffectKind::Notify,
        operation_id: "notify-pool-live-1".to_string(),
    };
    assert_eq!(
        WorkflowExecutor::begin_workflow_side_effect(&executor, &side_effect)
            .await
    ...
    WorkflowExecutor::complete_workflow_side_effect(
        &executor,
        &side_effect,
        Some(&serde_json::json!({"provider_id": "msg-pool-1"})),
    )
    .await
    .unwrap();
    assert_eq!(
        WorkflowExecutor::begin_workflow_side_effect(&executor, &side_effect)
            ...
            .unwrap(),
        WorkflowSideEffectStatus::AlreadyCompleted {
            result: Some(serde_json::json!({"provider_id": "msg-pool-1"}))
        }
    );

    let mut timed_out = WorkflowContext::new("wf-pool-timeout-live", "awaiting_vendor");
    timed_out.definition_name = Some("booking_recovery_pool".to_string());
    timed_out.set_cursor(WorkflowCursor {
        state: "awaiting_vendor".to_string(),
        frames: vec![WorkflowCursorFrame::Steps { index: 1 }],
        wait: Some(WorkflowPendingWait {
    ...
    let due = WorkflowExecutor::load_due_workflow_timeouts(
        &executor,
        "booking_recovery_pool",
        Utc::now(),
        10,
    )
    .await
    .unwrap();
    assert_eq!(due, vec!["wf-pool-timeout-live".to_string()]);
}
